...
> ⚠️ **历史方案，非现状（2026-09-10 标注）**: 本文是 2026-04-08 的立项设计，**实现时偏离了以下部分**，请勿按本文判断当前技术栈：
> - **未使用 LangChain / LangGraph** —— 编排、状态机、重试全部自研，依赖官方 `openai` / `anthropic` SDK；
> - **未使用 ChromaDB / SQLite 存储** —— 记忆系统曾实现但从未被主流程调用，已于 2026-09-10 删除；
> - **ReAct（观察-思考-行动）骨架** —— `observe/think/act` 方法已移除，改为「抽象 `process()` + 各 Agent 实现」。
>
> 当前状态请见 [PROGRESS.md](../../../PROGRESS.md)。
